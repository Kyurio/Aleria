<nav class="navbar navbar-expand-lg navbar-light bg-light sticky-top mb-5 px-5">
  <a class="navbar-brand" href="<?php echo RUTA_URL ?>"><?php echo NOMBRE_SITIO; ?></a>

  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>

  <div class="collapse navbar-collapse"  id="navbarNav">
    <ul class="navbar-nav ml-auto">
      <li class="nav-item">
        <a class="nav-link" href="<?php echo RUTA_URL ?>">Home</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="<?php echo RUTA_URL ?>pages/productos">productos</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="<?php echo RUTA_URL ?>pages/nosotros">Nosotros</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="<?php echo RUTA_URL ?>pages/nosotros">Blog</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="<?php echo RUTA_URL ?>pages/contacto">contacto</a>
      </li>
    </ul>
  </div>
</nav>
